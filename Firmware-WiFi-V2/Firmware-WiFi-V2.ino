...
PROGMEM const char CLAVE[] = "42314212";
...
//Direcciones asociadas al modulo wifi
byte dirIP[4];
byte dirMAC[6];
...
//Cadena que contendra la solicitud GET con los reportes
char reporte[256];
...
void setup(void) {
  ...
  Serial.begin(9600);
  esp8266_inicializar(&serieWifi, pinEspEn);
  esp8266_modoCliente(FSH(AP), FSH(CLAVE));

  for (;;) {
    esp8266_hacerTareas();

    if (esp8266_estadoNuevo() && esp8266_leerEstado() == EE_WIFI_ARRIBA) {
      esp8266_leerIP(dirIP);
      esp8266_leerMAC(dirMAC);
  ...
  for (;;) {
    ...
    //    if (esp8266.reset()) { Serial.println(F("Inicializado")); break; }
  ...
  for (;;) {
    ...
    //        { Serial.println(F("Conectado")); break; }
    //      else
...
void loop(void) {
  ...
  sprintf(strValor, "%i.%02i", valor / 100, valor % 100);
  //      enviarDato(F("Temperatura"), strValor);
  anexarDato(F("Temperatura"), strValor);
...
void iniciarReporte() {
  //Se genera la cabecera HTTP/GET
  strcpy_P(reporte, PSTR("GET "));
  strcat_P(reporte, pagina);
  strcat_P(reporte, PSTR("/input/post.json?apikey="));
  strcat_P(reporte, apikey);
  strcat_P(reporte, PSTR("&node="));
  strcat(reporte, idNodo);
  strcat_P(reporte, PSTR("&json={"));
...
void anexarDato(const __FlashStringHelper *nombre, char *valor) {
  //Si la bandera de primer dato esta activa omite la coma, sino la anexa
  if (primerDato)
    primerDato = false;
  else
    strcat_P(reporte, PSTR(","));

  //Se anexa el par "variable:valor" al reporte json
  strcat_P(reporte, (PGM_P) nombre);
  strcat_P(reporte, PSTR(":"));
  strcat(reporte, valor);
}

void cerrarReporte() {
  //Genera el resto de la cabecera HTTP/GET
  strcat_P(reporte, PSTR("} HTTP/1.1\r\nHost: "));
  strcat_P(reporte, servidor);
  strcat_P(reporte, PSTR("\r\nConnection: close\r\n\r\n"));

  //Imprime el reporte en la terminal serie asi como su longitud. Si la misma
  //excede el 90% de la capacidad del buffer, se emite una advertencia.
  Serial.print(reporte);
  Serial.print(F("Longitud del reporte: "));
  Serial.print(strlen(reporte));
  Serial.print(F(" bytes"));
  if (strlen(reporte) < sizeof(reporte) * 9 / 10)
    Serial.println();
  else
    Serial.println(F(" - ADVERTENCIA: Mas del 90% del buffer esta siendo usado"));
}
...
bool enviarReporte() {
  ...
  /*
    ...
    //Envia la solicitud GET al servidor con el reporte de los sensores
    if (!esp8266.enviar(F("GET / HTTP1.1\r\n\r\n"))) {
    //if (!esp8266.enviar(reporte)) {
      Serial.println(F("Error al enviar datos"));
      esp8266.desconectar();
      return false;
    }
